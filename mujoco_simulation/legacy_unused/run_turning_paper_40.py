...
SUMMARY_CSV = LOG_DIR / "turning_paper_40_summary.csv"
STATUS_PATH = LOG_DIR / "turning_paper_40_status.json"
PAPER_REWARD_DIR = OUTPUTS / "csv_png" / "paper_reward_summary"
...
def build_jobs(runs_per_condition: int) -> list[dict]:
    conditions: list[dict] = []
    for mode in ("yaw", "radius"):
        for direction in ("left", "right"):
            target = 0.5
            mode_code = "y" if mode == "yaw" else "r"
            base = f"paper_ppo_turn_{direction}_a20_{mode_code}05"
            condition = {
                "mode": mode,
                "direction": direction,
                "target": target,
...
def write_reward_summary_plot() -> Path:
    ...
    return plot_groups(groups, PAPER_REWARD_DIR, n_bootstrap=10_000, seed=20260618)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the paper yaw/radius 0.5 turning PPO batch: 4 conditions x 10 runs.")
...
def main() -> None:
    ...
    conditions = build_jobs(args.runs_per_condition)
