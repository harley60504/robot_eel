...
#define CENTER_PACKET_HEADER 0xAC
...
static inline void sendCenterPacketUART(
  ...
  CenterPacket pkt;
  pkt.header = CENTER_PACKET_HEADER;
  pkt.count  = count;
  pkt.save   = save ? 1 : 0;
  pkt.seq    = seq;
  ...
  pkt.checksum = calcPacketChecksum(
    ...
    sizeof(CenterPacket) - 1
  );

  serial.write(reinterpret_cast<uint8_t*>(&pkt), sizeof(CenterPacket));
}
...
static inline bool feedCenterRx(CenterRxState &st, uint8_t b) {
  ...
  if (st.index >= sizeof(CenterPacket)) {
    st.receiving = false;
